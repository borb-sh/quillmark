...
### 6.2 System metadata: `Sentinel` removed, `SystemMeta` added

A block's leading `#@key: value` lines are now plain **system metadata**, not
validated positional sentinels. The API reflects this:

- The `Sentinel` enum is **removed**.
- A new type **`SystemMeta`** is exported — it holds a block's `#@` metadata
  as an ordered key/value map.
- `Card::sentinel()` is replaced by:
  - `Card::meta()` — the block's `SystemMeta` map.
...
- [ ] Replace `Card::sentinel()` / `Sentinel` usage with `Card::meta()` /
      `kind()` / `id()` and the `SystemMeta` type; drop any
