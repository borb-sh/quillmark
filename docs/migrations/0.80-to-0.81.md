# 0.80.0 → 0.81.0 — card-yaml syntax

`0.81.0` replaces Quillmark's Markdown metadata syntax with the **card-yaml**
format. This is a **hard cutover**: the `0.80.0` `---`/`QUILL:` frontmatter and
the ` ```card <kind> ` fenced cards (and the legacy `---`/`CARD:` fence) are
removed and no longer parse. Stored documents must be migrated.

The class and function APIs — Rust, WASM, and Python — are **unchanged**. Only
the document *syntax* changes.

## TL;DR

Every metadata block is now a `~~~card-yaml` fence carrying a `#@`-prefixed
**system sentinel**:

```diff
- ---
- QUILL: my_quill
- title: Main Document
- ---
+ ~~~card-yaml
+ #@quill: my_quill
+ title: Main Document
+ ~~~

  Some content here.

- ```card products
- name: Widget
- price: 19.99
- ```
+ ~~~card-yaml
+ #@kind: products
+ name: Widget
+ price: 19.99
+ ~~~

  Widget description.
```

Why: isolating the structured payload inside an explicitly delimited block —
separate from the prose body that follows it — keeps LLM generation stable and
prevents state corruption. A generator editing prose cannot disturb the
structured fields, and vice versa.

## 1. The card-yaml block

A card-yaml block has four parts, in order:

1. **Opening fence** — exactly `~~~card-yaml` (three tildes, at column zero).
2. **System sentinel** — the first non-blank line inside the block, prefixed
   with `#@`.
3. **Data payload** — standard YAML key/value pairs below the sentinel.
4. **Closing fence** — exactly `~~~`.

The prose body begins immediately after the closing `~~~` and runs to the next
block or end of document.

### System sentinels

The `#@` prefix keeps the sentinel out of the YAML payload (a bare `#` line is
an ordinary YAML comment).

- The document's **first (root) block** must declare `#@quill: <name>@<version>`.
  This replaces the old `QUILL:` frontmatter key.
- Every **subsequent block** is a card and must declare `#@kind: <kind>`. This
  replaces the ` ```card <kind> ` info string and the legacy `CARD:` key.

The sentinel must be the first non-blank line of the block. `#@quill:` is
permitted only in the root block; `#@kind:` is required in every other block.

### Blank-line rule

A blank line is required immediately above every `~~~card-yaml` opener, except
when the opener is the very first line of the document. A `~~~card-yaml` line
without a blank line above it is treated as an ordinary CommonMark fenced code
block, not a metadata block (a `parse::card_fence_missing_blank` warning is
emitted).

## 2. Migrating stored documents

For each document:

1. Replace the opening `---` of the frontmatter with `~~~card-yaml`, and rename
   the `QUILL:` line to `#@quill:`. Replace the closing `---` with `~~~`.
2. For each card written as ` ```card <kind> `, replace the opener with
   `~~~card-yaml` followed by a `#@kind: <kind>` line, and replace the closing
   ` ``` ` with `~~~`.
3. For each legacy card written as a `---` block with a `CARD:` key, replace
   the fences with `~~~card-yaml` / `~~~` and rename `CARD:` to `#@kind:`.
4. Ensure every `~~~card-yaml` opener has a blank line above it.

A bare `---` in body prose was already an ordinary CommonMark thematic break;
it is unaffected and needs no change.

`Document::to_markdown()` (`doc.toMarkdown()` in WASM, `doc.to_markdown()` in
Python) emits the **canonical card-yaml form**, so re-serializing a parsed
document produces compliant output automatically.

## 3. What is unchanged

- **Field names** still match `[a-z_][a-z0-9_]*`; **card kinds** still match
  `[a-z_][a-z0-9_]*`.
- `QUILL`, `CARD`, `BODY`, and `CARDS` remain **reserved** field names.
- The plate wire format (`QUILL`, `BODY`, `CARDS`, per-card `CARD`) is
  unchanged — backends see the same JSON shape.
- YAML payload **comments** (own-line and inline `field: value  # note`) and
  the **`!fill`** placeholder tag are still supported and still round-trip.
- The `Document`, `Card`, `Frontmatter`, `Quill`, and rendering APIs are
  unchanged across Rust, WASM, and Python.

## 4. What is removed

- The `---`/`QUILL:` frontmatter block.
- The ` ```card <kind> ` fenced-card syntax and the legacy `---`/`CARD:` card
  fence.
- **Comments on the sentinel line.** The old `QUILL: ref  # note` inline
  comment has no equivalent — the `#@` sentinel line carries no trailing
  comment. Move such notes to an own-line comment in the payload.
- The "near-miss sentinel" lint and the "cannot specify both QUILL and CARD"
  error — neither situation can arise under card-yaml.

## 5. Error messages

Parse errors are reworded for the new syntax. Match on `ParseError` variants
(or diagnostic codes) rather than message text:

| Situation | Variant |
|---|---|
| No `~~~card-yaml` block, or the root block has no `#@quill:` | `MissingQuillField` |
| Root block declares `#@kind:`, or a later block declares `#@quill:` | `MissingQuillField` / `InvalidStructure` |
| A block's first non-blank line is not a `#@` sentinel | `MissingQuillField` / `InvalidStructure` |
| `~~~card-yaml` opener with no `~~~` closer | `InvalidStructure` |
| Invalid card kind | `InvalidStructure` |
| Reserved name used as a field | `InvalidStructure` |
| Invalid YAML payload | `YamlErrorWithLocation` |

## Checklist

- [ ] Migrate stored Markdown from `---` frontmatter / `card` fences to
      `~~~card-yaml` blocks with `#@` system sentinels.
- [ ] Ensure every `~~~card-yaml` opener has a blank line above it.
- [ ] Move any inline comment that sat on a `QUILL:` / `CARD:` line into an
      own-line payload comment.
- [ ] Regenerate snapshot/golden fixtures that compare `to_markdown()` output.
- [ ] Update error handling that matched on old parse-error message text.
