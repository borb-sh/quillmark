...
- **Blank-canvas constructor** — `Document::new(quill_ref)` (Rust),
  `Document(quill_ref)` (Python), `new Document(quillRef)` (WASM, .NET): a
...
- **Atomic batch mutation** — `Card::set_fields` (Rust);
  ...
  `doc.setFields(obj)` / `doc.updateCardFields(index, obj)` (WASM);
  `doc.SetFields(dict)` / `doc.UpdateCardFields(index, dict)` (.NET). The
