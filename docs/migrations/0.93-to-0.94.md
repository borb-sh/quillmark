...
Two field types join the schema. Both are additive — existing quills load
unchanged — but they narrow what `string` is for and give the taxonomy a clean
2×2: **enum** (closed data), **string** (open data), **plaintext** (plain
content), **richtext** (formatted content).

### `plaintext` — navigable prose that takes no formatting

`plaintext` is a **content** type: it rides the same `RichText` corpus as
`richtext` (same `contentMediaType`, so it lowers through the identical backend
path and inherits preview↔editor navigation and per-field regions), but it is
authored and projected through a **literal** codec, not markdown. `*hi*` is four
literal characters, verbatim both ways — no emphasis, no escaping, an idempotent
round-trip. Declare it exactly like richtext, including the sibling `inline:`
key:

```
subject:
  type: plaintext          # a navigable, unformatted single line
  inline: true
...
A lone `\n` is a within-paragraph line break; a blank line (`\n\n`) is a
paragraph boundary. A plaintext value carries no marks, islands, or block
formatting (lists, quotes, headings); a wire corpus that does is **rejected**,
not silently stripped (`plaintext::not_plain`), mirroring the `inline`
precedent. In the transform schema a plaintext field carries the richtext media
type plus `quillmark:plain: true` (an editor hint; backends ignore it).

Do not confuse the **`plaintext` field type** with the **`plaintext(field)`
Typst helper** above: the helper projects *any* corpus (richtext or plaintext)
to a `str`; the field type declares that a field's content is plain to begin
with. A plaintext field still reaches the plate as Typst **content**, so the
`str`-consumer caveat from the richtext migration applies — use the
`plaintext(field)` helper where a plate needs the `str`.

### `enum` — a first-class closed string domain

`enum` is promoted from a `string` modifier to a type token. Declare
`type: enum` with a required `values:` list:
...
The token projects to the idiomatic JSON-Schema `{type: "string", enum: [...]}`
— exactly what backends already dispatch on (a string plus its domain), so no
backend or plate change is needed. The `enum:` modifier on `type: string` is
**still accepted for this release** (both spellings populate the same store and
validate identically) but is deprecated; migrate to `type: enum`. Two behaviors
change at the edges:

- `enum:` (or the new `values:`) on any type that is **not** `string`/`enum` is
  now a **load error**, not a silent no-op — the old spelling validated nothing
  on, say, an `integer` field.
- `type: enum` **requires** a non-empty `values:` list; `values:` on a
  `string` is a load error (use the `enum:` modifier there, or switch to
  `type: enum`).

Scope is string-valued members in v1 — an enum is a branching key; numeric
domains are range constraints on `number`, not enums.

`string` itself is unchanged in spelling but narrowed in intent to **open scalar
data**: URLs, paths, identifiers, reference keys — anything a template computes
with (`#link(dest)` / `#image(path)`, comparison, keying) rather than lays out.
Reach for `plaintext` when the value is prose the author navigates, and `enum`
when it is drawn from a closed set.
