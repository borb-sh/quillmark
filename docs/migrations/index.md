...
| Step | What changes |
|---|---|
| [0.114 → 0.115](0.114-to-0.115.md) | A variant-bearing `enum` rests as the bare string every enum is, and each world's cells rest as card-level fields beside it: `classification: CUI` with `poc: …` at the same level, read as `data.classification` and `data.poc`, addressed as `main.poc`. The container spelling refuses to render, every cell is on the plate blank-filled on every document, a cell's name is unique across the card, and `VARIANT_DISCRIMINANT_KEY` is deleted. Then two copies of a fact an island already carried go. An island’s canonical form loses `loss`, and a block island’s line is a `para` — whether its slot’s markup is a block is read off the island’s `type`, where the mint and both emitters already read it. A host branching on `line.kind === 'island'` reads the island instead, and the `setKind` in every block-island bundle goes. `ContentIsland.loss`, `ContentLossClass`, `Loss`, `Island::with_loss` and `IslandType::default_loss` are deleted, and the closed content vocabularies drop to four. Canonical bytes move for every document holding an island or a table, so content hashes recompute and the tag becomes `quillmark/document@0.115.0`. Stored rows keep loading: the decoder reads `island` as the `para` it always projected, and ignores a `loss`, permanently. |
