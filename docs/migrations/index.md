...
No step requires migrating a stored document: the storage DTO's tag holds across
every step here except 0.111 → 0.112, where it becomes
`quillmark/document@0.112.0`, and its bytes hold except at that step and
0.114 → 0.115, where canonical bytes move and content hashes recompute.

## Guides

| Step | What changes |
|---|---|
| [0.114 → 0.115](0.114-to-0.115.md) | An island's canonical form loses `loss`, so canonical content bytes move for any document holding an island and a content hash over them recomputes — a stored row carrying the key still opens. `ContentIsland.loss`, `ContentLossClass`, `Loss`, `Island::with_loss` and `IslandType::default_loss` are deleted, and the closed content vocabularies drop to four. |
