...
| Step | What changes |
|---|---|
| [0.114 → 0.115](0.114-to-0.115.md) | Two copies of a fact an island already carried go. An island’s canonical form loses `loss`, and a block island’s line is a `para` — whether its slot’s markup is a block is read off the island’s `type`, where the mint and both emitters already read it. A host branching on `line.kind === 'island'` reads the island instead, and the `setKind` in every block-island bundle goes. `ContentIsland.loss`, `ContentLossClass`, `Loss`, `Island::with_loss` and `IslandType::default_loss` are deleted, and the closed content vocabularies drop to four. Canonical bytes move for every document holding an island or a table, so content hashes recompute and the tag becomes `quillmark/document@0.115.0`. Stored rows keep loading: the decoder reads `island` as the `para` it always projected, and ignores a `loss`, permanently. Also: a schema is loaded, not deserialized — `QuillConfig`, `CardSchema` and `FieldSchema` drop `Deserialize` and `FieldSchema::from_quill_value` turns crate-internal, leaving the loader's shape gate the only way to a config. All three still serialize. And a matrix roster is one flat mapping of `id: Title`: the group heading over a block of members reached the visual editor and nowhere else, so `MatrixGroup`, `QuillMatrixGroup`, the `group` key on every projected member and `quill::duplicate_matrix_member` all go. And a blueprint stops *naming* a variant-bearing enum's dormant worlds: each is its own cells, commented out under a `# when <MEMBER>:` header, so a consumer reading the `# when <MEMBER>: <fields>` name list reads the cells instead. And `quillmark validate` compiles the plate: it renders the empty document, the blueprint and the seed through the quill's backend, so a quill whose plate does not render exits `1` where it exited `0` — `--no-render` is the configuration read on its own. And a `matrix` member mapping takes the ordinary ladder: a mapping naming no `held` is unheld, where presence alone used to tick it, so `{ detail: X }` now means what `{ held: false, detail: X }` means. It is the release's one silent break — the value is well-typed under both readings — so documents storing a ticked member as a bare mapping need `held: true` spelled before upgrading. |
