...
- **`quill.seed_document()`** is the starter: one card per declared kind, bodies from `body.example`, every field absent. Hand it to a human or an editor to fill in; see [Blueprint & Seeding](../quills/blueprint.md).
