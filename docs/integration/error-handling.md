...
`parse::*` · `validation::*` · `quill::*` · `edit::*` (mutators) · `typst::*` · `acroform::*` · `pdf::*` (the AcroForm stamping spine both PDF-producing backends share) · `backend::*` · `engine::*`.
...
`parse::payload_not_mapping` is a root card-yaml block whose payload is YAML but not a mapping, usually a document opening with code fenced in `~~~`. It is located at the fence's line. Its `args` carry `actual` (`string`, `number`, `boolean` or `sequence`) and, when the fence has one, `info`, the info string.

`parse::missing_kind` is a warning on a card-yaml block after the root with no `$kind`: a card missing its kind line, or tilde-fenced code that reads as YAML. The block renders as a code block in the body above. It is located at the fence's line.
