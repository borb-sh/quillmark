...
- `[MARKDOWN_FILE]`: Path to markdown file with a main card (optional — when omitted, the quill's blueprint is rendered)

`<QUILL_PATH>` selects the local quill bundle used for rendering. `MARKDOWN_FILE` still requires a main `~~~card-yaml` card with a `#@quill` system metadata line during parsing.
