...
| `default`     | any               | no       | The value the **majority of authors want**. When the field is omitted, the default is filled in. **Declaring `default` makes the field Endorsed**: the blueprint renders the default plus a `; delete-ok` tag. Omitting `default` makes the field **Unendorsed**: the blueprint renders the `<must-fill>` sentinel and validation flags `validation::field_absent` at validate time (a non-fatal signal — the render path zero-fills an absent field). |
