...
Quillmark attributes ink on its own for a `richtext` or `plaintext` field, for a scalar read written in the plate or a module it imports (`#data.subject`), and for a date placed with `display`. A [form-field widget](typst-backend.md#binding-to-a-schema-field) opts in with `field:`. Everything else is unattributed until a `field-region` claims it.
