...
> **Scope: Tier 1 + storage + render.** Field I/O flows through `quill.writer(doc)` / `quill.reader(doc)` exclusively, the whole document through `reader.values()` / `writer.set_values(values)`; `Document` is quill-free data and structure (parse, the storage DTO, `insert_card` / `remove_card` / `move_card`, `remove_field`, the `card` / `seed_overlay` reads, `$ext` / `$seed`).
>
