...
| Form | Lanes |
|---|---|
| Seam: every `instance` spelled | `reader.getContent`, `getStored` on a body, `importMarkdown`, `rebase`, and the `Card` wire behind `document.main` / `cards` / `card(i)` / `removeCard` / `seedMain` / `seedCard` |
