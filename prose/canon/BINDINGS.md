...
Everything needing no instance stays a static export, unawaited: `MAIN_CARD_ADDR`, `assignInstances`, `isQuillmarkError`. So do `Engine`, `LiveSession` and the four writer/reader classes, which their **arguments** gate rather than the door — every `Engine` verb takes a `Quill` first, and the writer/reader constructors take both handles, so a caller who has not awaited cannot produce an argument to call them with. Holding them out of the gate keeps them tree-shakable, so the editor path drops the dispatcher it never calls.
