...
`QUILL_PATH` provides the local quill bundle used for rendering. `MARKDOWN_FILE` still requires a main `~~~card-yaml` card with a `#@quill` system-metadata line because parsing enforces it.
