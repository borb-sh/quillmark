...
`QUILL_PATH` provides the local quill bundle used for rendering. `MARKDOWN_FILE` still requires a root `~~~card-yaml` block with a `#@quill:` sentinel because parsing enforces it.
