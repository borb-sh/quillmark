...
Quillmark converts Markdown with card-yaml metadata cards into output artifacts (PDF, SVG, PNG, TXT). A `Quill` (the renderable shape) orchestrates the pipeline; backends do the heavy compilation.

## Data Flow

1. **Parse** — card extraction, bidi stripping, HTML fence normalization
...
- **`Document`** — Typed in-memory representation of a Quillmark Markdown file (main card, body, cards)
