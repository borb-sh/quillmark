...
- **Typst emit**: walk lines grouped by container path → markup, escaping
  text runs as today; record generated ranges **at every container level**
  (line ⊂ item ⊂ list ⊂ field). Mixed-granularity windows give marker glyphs
  an honest home: a bullet's item-node span nests in the *item* window, so
  `field_at` on a bullet answers with the item range instead of nothing. The
  hierarchy-aware scan rework this requires is the same one any sub-field
  region design needs (grounding §3.2) — it is not a cost of this model.

Regions key on `(field, corpus range, revision)`. Paragraph-level preview
navigation — #829's goal — falls out of the per-line windows.
...
1. `Content` type + canonical serialization + markdown⇄corpus codecs +
   ...
   corpus.
2. Engine consumes `Content`; Typst emit from the corpus with hierarchical
   windows; regions re-key; storage cutover.
