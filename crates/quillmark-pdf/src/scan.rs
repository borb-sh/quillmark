//! Minimal byte-level PDF scanner and incremental-update writer. Not a general
//! PDF parser: it reads just enough of a producer's output (xref table, trailer,
//! catalog, page tree) to append a single AcroForm revision. Backs [`crate::stamp`].
//!
//! This is the proven `pdf_scan` machinery lifted out of the Typst backend so
//! every backend that stamps widgets shares one reader/appender. A pure-Rust
//! reader (`hayro-syntax`) is a candidate replacement (see issue #744) once it's
//! confirmed to expose enough for an incremental-update append.
...
/// Bail if the producer emitted an xref stream instead of a traditional table.
...
/// in document order. The recursion is capped to prevent runaway on a
/// pathological PDF.
...
/// error — the producers we target emit inline arrays).
pub(crate) fn rewrite_page_with_annots(
    pg_dict: &[u8],
    widget_refs: &[u32],
) -> Result<Vec<u8>, RenderError> {
