...
//! [`LiveSession`](crate::session::LiveSession) via
//! [`regions`](crate::session::LiveSession::regions) and
//! [`field_at`](crate::session::LiveSession::field_at).
//!
...
//! ([`RenderOptions::regions`](crate::types::RenderOptions)). Empty for
//! backends that place no schema fields.

/// One schema field placement's extent on a rendered page.
///
/// `rect` is `[x0, y0, x1, y1]` in PDF points with a **bottom-left** origin:
/// the lower-left corner of the page as drawn, which is also the origin of the
/// page raster and of
/// [`page_size_pt`](crate::session::LiveSession::page_size_pt). On a PDF
/// background that corner is the page's canvas box (`/CropBox` ∩ `/MediaBox`)
/// rather than user-space `(0, 0)`, so a widget's `/Rect` in the stamped PDF
/// and its region here differ by that box's origin.
///
...
///
/// Pass the output of
/// [`LiveSession::regions`](crate::session::LiveSession::regions) or a one-shot
/// [`RenderOptions::regions`](crate::types::RenderOptions) sidecar;
/// [`LiveSession::field_boxes`](crate::session::LiveSession::field_boxes) reads
/// the session's own. The union is a per-page *bounding* box, so it does cover
...
/// [`position_at`](crate::session::LiveSession::position_at) direction, paired
/// with [`locate`](crate::session::LiveSession::locate) (content position →
/// caret rect).
///
