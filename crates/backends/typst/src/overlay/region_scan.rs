//! SPIKE (#773): recover schema-field regions for *auto-tagged content* by
//! walking the compiled Typst frame tree, not by reading a fixed-size widget
//! box.
//!
...
//! many lines and break across pages. So its geometry can only be read from the
//! laid-out frames.
//!
//! Mechanism, mirroring `typst_layout::introspect::discover_frame` (the same
//! transform composition Typst uses to assign element positions):
//!   1. The helper wraps each auto-tagged content field between two zero-size
//!      `<__qm_region__>` metadata markers (`role: start` / `role: end`,
//!      carrying the schema `field` path).
//!   2. Query the markers → `location → (role, field)`.
//!   3. Walk every page frame in document order, composing the group transform
...
//!   4. Flip each page-space (top-left) union to the PDF bottom-left origin the
//!      region model uses.
//!
...
//! span the entire paragraph, which is undesirable").
...
/// alongside the (already-validated) form-field path.
