...
//! and inverts the owning run's escape scan to a cluster-exact corpus offset
//! (degrading to segment start where a node nests in no single run — a
//! multi-line `#raw` block); [`locate`] forward-maps a corpus offset to a
...
use quillmark_core::{CorpusHit, RenderedRegion};
...
pub(crate) struct FieldWindow {
    ...
    /// segment's corpus range and navigation ([`position_at`]/[`locate`])
...
pub(crate) fn scan(
    ...
    for (ki, &(wi, seg)) in keys.iter().enumerate() {
        ...
        // A content segment carries its corpus range; a segment-less window
        // (scalar/widget site) carries none.
        let span = seg.map(|s| {
            let c = &window.segments[s].corpus;
            [c.start, c.end]
        });
...
/// One drawn glyph (or shape/image) carrying enough to resolve a corpus
...
/// USV offset. Degrades to the segment's corpus start when the resolved node
...
/// content ink or on scalar/widget ink (no corpus address). `x`/`y` are PDF
...
/// The corpus USV offset a glyph's generated position maps to. The **owning
/// run** is the one whose `gen` range contains the whole resolved node; absent
/// (the `#raw` multi-line case, where every line shares one node wider than any
/// run, or structural ink) the safe degrade is the **segment**'s corpus start,
/// not a node-start computation that could land outside every line's own text.
fn invert_hit(helper: &Source, segmap: &SegmentMap, node: &Range<usize>, offset: u16) -> usize {
    let Some((corpus_r, gen_r, ctx)) = segmap
        .runs
        .iter()
        .find(|(_, g, _)| g.start <= node.start && node.end <= g.end)
    else {
        return segmap.corpus.start;
    };
    ...
    corpus_r.start + crate::emit::invert_gen_offset(gen_text, *ctx, abs - gen_r.start)
}

/// A corpus position → **caret rect**. Find `field`'s content window and the
...
pub(crate) fn locate(
    ...
    let seg_idx = window
        .segments
        .iter()
        .position(|s| s.corpus.start <= pos && pos <= s.corpus.end)?;
...
/// The generated byte a corpus position forward-maps to: the run containing
