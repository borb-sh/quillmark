...
fn update_commits_and_dirties_only_the_touched_suffix() {
    ...
    let mut session = backend
        .open(&q, &json!({ "msg": msg(n, None, "") }), common::test_date())
        .expect("open");
...
fn update_with_reordered_fields_same_content_is_clean() {
    ...
    let mut session = backend.open(&q, &opened, common::test_date()).expect("open");
...
fn update_is_transactional_on_compile_failure() {
    ...
    let mut session = backend
        .open(&q, &json!({ "msg": "last good" }), common::test_date())
        .expect("open");
    let pages = session.page_count();
...
fn update_tracks_page_count_growth_and_shrink() {
    ...
    let mut session = backend
        .open(&q, &json!({ "msg": msg(4, None, "") }), common::test_date())
        .expect("open");
