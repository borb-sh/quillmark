...
/// A leaf printed through its container's `ink` keeps its address past every
/// shape a plate read loses it to: a function parameter, a loop variable, a
/// destructuring, a date formatted inside a helper.
#[test]
fn ink_keeps_the_address_through_functions_loops_and_patterns() {
    let session = open(
        "#import \"@local/quillmark-helper:0.1.0\": data, display, ink\n\
         ...
         #for r in data.refs [#ink(r).org / #ink(r).num]\n\
         #let (poc, ..) = ink(data.classification)\n\
         #poc\n\
